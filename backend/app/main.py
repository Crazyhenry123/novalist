...
async def agent_invoke(request: Request):
    ...
    The request body contains the user's message plus connection_id and
    callback_url for sending streaming responses back through API Gateway.
    """
    ...
    # Build a send_message callback that posts to API Gateway WebSocket
    apigw = None
    if callback_url:
        apigw = boto3.client(
    ...
    async def send_message(msg: dict):
        """Send a streaming message back to the client via WebSocket."""
    ...
    if action == "start_novel":
        ...
        try:
            ...
            await send_message({"type": "error", "content": f"Invalid request: {e}"})
            return {"status": "error", "message": str(e)}

        user_id = body.get("user_id", "system")
        result = await run_novel_pipeline(novel_req, send_message, user_id)
        ...
        await send_message({
            "type": "info",
            "content": f"Chapter {chapter_num} generation for novel {novel_id} — use start_novel for full pipeline.",
        })
        return {"status": "ok"}

    elif action == "ping":
        await send_message({"type": "pong", "content": "alive"})
        return {"status": "ok"}

    else:
        await send_message({"type": "error", "content": f"Unknown action: {action}"})
